exports = module.exports = opts => opts.buffer ? exports.buffer : exports.string
...
function create (buffer) {
  ...
  function cmp (val, offset, eq, join) {
    ...
    if (val.length === 1) {
      return `s.charCodeAt(${ptr(offset)})${eq}${code(val)}`
    }

    return `s.slice(${ptr(offset)}, ${ptr(offset + val.length)})${eq}${JSON.stringify(val)}`
  }
}
