...
function defaults (opts) {
  ...
  const fullMatch = !!opts.fullMatch
  const sw = switchDefaults(opts)
  const { ptr, eq, ch, code } = opsDefaults(opts)
  ...
  function compileObject (gen, prop, schema, genany) {
    // schema is {..., fields: [...]}
    const fields = schema.fields || []
    const assigning = !prop || !prop.getable
    const allowEmpty = !fields.some(isRequired)
      && schema.allowEmpty !== false
      && allowEmptyObjects

    const o = assigning ? gen.sym(schema.name || 'o') : prop.get()
    const reqs = new Required(gen, o, fields)

    if (assigning) {
      defaultObject(gen, o, schema)
    }
    ...
    if (!fields.length) {
      ...
      `)
    } else if (bool(schema.ordered, ordered)) {
    ...
    if (!prop) gen(`return ${o}`)
    else if (assigning) prop.set(o)
  ...
  function defaultObject (gen, name, schema) {
    gen(`const ${name} = {`)
    defaultFields(gen, schema.fields)
    gen('}')
  }
